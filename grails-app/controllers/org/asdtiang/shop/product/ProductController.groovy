...
import org.asdtiang.grails.service.AccessService
import org.hibernate.FetchMode

import org.springframework.dao.DataIntegrityViolationException

class ProductController {
    def  AccessService accessService
    ...
    def show() {
        if(params.key){
                if(accessService.authKey(params.key)==true){
                    def productInstance = Product.get(Long.parseLong(params.id))
                    if (!productInstance) {
                        render accessService.genResult(null,"not found product [id:${id}]")
                    }
                    else{
                        render  accessService.genResult(productInstance)
                    }
            }
        }
        def productInstance = Product.get(Long.parseLong(params.id))
