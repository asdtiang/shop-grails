	...
	<body>
		...
		<div id="show-comment" class="content scaffold-show" role="main">
			...
			<ol class="property-list comment">
				...
				<g:if test="${commentInstance?.product}">
				<li class="fieldcontain">
					<span id="product-label" class="property-label"><g:message code="comment.product.label" default="Product" /></span>
					
						<span class="property-value" aria-labelledby="product-label"><g:link controller="product" action="show" id="${commentInstance?.product?.id}">${commentInstance?.product?.encodeAsHTML()}</g:link></span>
